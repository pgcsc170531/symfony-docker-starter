...
use App\Entity\Tenant\School as TenantSchool; // 👈 Import Tenant School for settings
...
class NotificationService
{
    ...
    // This swaps tags like [student_name] or [class] with real data
    $message = strtr($template, $placeholders);

    // 2. CHECK TENANT TOGGLES FIRST (Efficiency)
    $tenantSettings = $this->tenantEm->getRepository(TenantSchool::class)->findOneBy([]);
    
    $shouldSend = match($eventType) {
        'enrollment' => $tenantSettings?->isSmsOnEnrollment() ?? true,
        'fees'       => $tenantSettings?->isSmsOnFeePayment() ?? true,
        'calendar'   => $tenantSettings?->isSmsOnCalendarEvent() ?? true,
        default      => false
    };
